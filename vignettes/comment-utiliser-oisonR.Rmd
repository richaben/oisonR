...
Cette fonction permet de consulter/récupérer (selon l'argument `collect_all = FALSE` ou `collect_all = TRUE`) les données sur une zone géographique définie par un polygone. Spécifier une plage de date est également possible. Le polygone peut être :
...
```
  ...
  - tracé à la volée lorsque l'argument `draw_zone = TRUE` est spécifié dans la fonction. Dès lors, un outil de sélection de zone s'ouvre (via une fenêtre interactive `shiny`) permettant le traçage du polygone et son. La forme stockage dans l'environnement de travail pour la requête.
